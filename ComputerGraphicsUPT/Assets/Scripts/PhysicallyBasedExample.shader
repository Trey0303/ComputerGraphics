    ...
    Properties
    {
        ...
        SubShader
        {
            ...
            Varyings LitPassVertex(Attributes input)
            {
                ...
                //equation for wobble effect
                //y = sin(x * (float range)Frequency + time) * 
                input.positionOS.y += sin(input.positionOS.x * _Frequency + _Time.y) * _Amplitude;//wobble effect

                input.positionOS.xyz *= 1.5;//objects scale


                //gradientNoise(input.positionOS.xyz * 1) + 0.5;
